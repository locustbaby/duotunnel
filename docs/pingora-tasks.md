...
5. H1 downstream 的 upstream request 现已统一走 `HttpConnector::request()`，不再由 `HttpPeer` 直接持有 `HttpsClient` 发请求；cleartext upstream 的 `prefer_h1` 记忆因此同时覆盖 H1/H2 两条 live request path。
6. cleartext upstream 的空 body 请求在 h2c 失败后会自动回退到 H1 一次；非空 body 请求仍然只做偏好记忆，不冒险重放。
7. H1 keep-alive loop 仍留在 `HttpPeer::connect_inner`，尚未下沉到 session 层；这一尾巴已拆到 TODO-67b。
...
7. h2c cache value 现已从拆散的 `{ conn_id, conn, sender }` 收口到 `CachedSender { selected: Arc<SelectedConnection>, sender }`，失效判断和条件删除都直接挂在 `selected` 上；这部分已和 TODO-70 对齐。
8. CI 辅助：`ci-helpers/run-bench-case.sh` 的 frp 路径加了端口清理（`pkill` + `reset-failed`）、基于 `is-failed` 的就绪等待、启动失败 diag dump 与 3 次重试，避免 bench 偶发被残留 frpc/frps 卡住。
9. 但 per-route state 仍主要是 handler 内部的 `Mutex<HashMap<...>>` 组合，尚未进一步收敛成文档里建议的 `H2cConnState` / `DashMap` 结构；完整 failover / replayability 语义也还没收尾。
...
**Problem**:
这个任务原本属于对齐性/小优化项。当前代码已完成：server 侧 snapshot 已与 client 一样切到 `ArcSwap<Vec<Arc<SelectedConnection>>>`，h2c plugin 的 cached sender 也已随之对齐。
...
**实际进展（已完成）**:
1. `server/registry.rs` 的 `snapshot` 已改为 `ArcSwap<Vec<Arc<SelectedConnection>>>`。
2. `ClientGroup::build_snapshot` / `select_healthy` / `ClientRegistry::select_client_for_group` 已全部返回 `Arc<SelectedConnection>`。
3. `server/plugins/h2c/mod.rs` 的 cached sender 已同步持有 `Arc<SelectedConnection>`。
4. `server/plugins/tls/mod.rs` 等调用方已适配 `Arc<SelectedConnection>` 返回值。

**Validation**: `cargo check -p tunnel-lib -p server -p client` 已通过。单元测试与 flamegraph 收益可在后续有基准窗口时补齐。

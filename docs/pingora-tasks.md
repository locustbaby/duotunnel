# Pingora-inspired 重构：Task 清单（2026-04-21 修订）
...
- **UpstreamResolver trait** `tunnel-lib/src/proxy/core.rs:23-28`：`async fn upstream_peer(&self, ctx: &mut Context) -> Result<PeerKind>`
...
- `UpstreamPeer` trait 统一（`PeerKind` enum 仍在）
- `ClientId/GroupId/ReuseHash` newtype
- 结构化 `Error` + `RetryType::ReusedOnly`
- `HttpConnector`（H1/H2 降级记忆）
- h2c per-route sticky sender cache 的并发化、失效重选与错误语义
- P2C pick 算法
- 若干 client 侧小优化
...
| TODO | 标题 | Priority | 状态 |
|---|---|---|---|
| [TODO-63](#todo-63-upstreampeer-trait-统一--消灭-peerkind-分派不一致--主线第一步) | UpstreamPeer trait 统一 + 消灭 PeerKind 分派不一致 | High | TODO |
| [TODO-64](#todo-64-reusehash--clientidgroupid-newtype--主线第二步) | ReuseHash + ClientId/GroupId newtype | High | TODO |
| [TODO-65](#todo-65-tunnel-lib-error-类型--retrytypereusedonly--主线第三步) | tunnel-lib Error 类型 + RetryType::ReusedOnly | High | TODO |
| [TODO-66](#todo-66-统一-httpconnector--h1h2-降级记忆替代-httppeerh2peer) | 统一 HttpConnector + H1/H2 降级记忆 | High | TODO |
...
| [TODO-69](#todo-69-h2c-per-route-sticky-cache-并发化--failover) | h2c per-route sticky cache 并发化 + failover | Medium | TODO |
| [TODO-70](#todo-70-server-端-snapshot-持-arcselectedconnection对齐-client) | Server 端 snapshot 持 Arc\<SelectedConnection\>（对齐 client） | Medium | TODO |
...
**推荐落地顺序**：70 → 72 → 65 → 66 → 63 → 67b → 68 → 69 → 64 → 71。理由：先做低风险的 server snapshot / client retry 小优化；再引入结构化错误；随后在现有 hyper client 基础上做协议偏好记忆；`PeerSpec` 和 newtype 这类大范围类型重构放到行为稳定后推进。每个 TODO 完成后跑 CI stress phase，观察基准变化。
...
pub trait UpstreamPeer: Send + Sync {
    ...
    fn reuse_hash(&self) -> ReuseHash;     // 见 TODO-64
...
具体动作：
1. 建 trait + 两种实现（BasicPeer / HttpPeerSpec / 保留 `MitmPeerSpec` 作为 client MITM 路径）
2. 拆掉 `HttpPeer` / `H2Peer` 的连接逻辑，下沉到 TODO-66 `HttpConnector`
3. 把 `PeerKind::Dyn` 合并回常规 trait —— 不再保留 enum + trait 双轨
4. 容器统一为 `Arc<dyn UpstreamPeer>` 或具体 enum（按场景）
5. `UpstreamResolver::upstream_peer` 返回类型从 `PeerKind` 改为 `Arc<dyn UpstreamPeer>`

**为什么好**（对比现状）：

| 维度 | 现状 | 新设计 |
|---|---|---|
| 分派一致性 | enum + `Dyn` 后门双轨 | 单一 trait |
| 扩展新协议（e.g. MITM 变体） | 改 enum + 改所有 match | 新增 `impl UpstreamPeer` 即可 |
| 连接管理语义 | peer 与 connector 职责混杂 | Peer 只描述上游，Connector 按 reuse_hash / preference 管理连接 |
...
**Validation**: CI 全量（含 stress phase）无回归；metrics `request_completed` 各协议正常；client MITM H2 路径继续可用。
...
- duotunnel 的上游连接池目前依赖 `hyper_util` 内部 key（大体围绕 scheme/authority）；对于未来自定义 connector / per-peer preference，仍缺一个显式 `ReuseHash` 来把 address、scheme、ALPN、TLS 设置纳入池 key。注意：这不是当前已确认的线上错误复用 bug，而是 TODO-66 自定义连接管理前需要补齐的类型边界。
...
`UpstreamPeer::reuse_hash()` 默认实现（示意；可先用 `std::collections::hash_map::DefaultHasher`，除非 profile 证明需要引入 `ahash`）：
```rust
fn reuse_hash(&self) -> ReuseHash {
    ...
    if let Some(o) = self.options() {
        o.alpn.hash(&mut h);
        o.tls.as_ref().map(|t| t.reuse_key.hash(&mut h));
    }
    ReuseHash(h.finish())
}
```
...
分阶段替换：
1. 内部热路径先替换：`server/registry.rs`、`server/control_client.rs`、`server/plugins/h2c/mod.rs` 的 `RouteTarget` / cache key、`tunnel-lib/src/plugin/ctx.rs` 的 `Route`。
...
替换关键路径：`ctld_proto.rs`、`open_bi.rs`、`tunnel-store/traits.rs`、`HttpPeer/H2Peer`（和 TODO-66 一起整）、`h2c` plugin 的 502 路径。
...
**Fix**:
分两阶段做，先不要直接自研完整 H1/H2 pool 替代 hyper。
...
- `tunnel-lib/src/proxy/http.rs` 和 `h2.rs` 先改为调用 connector / 结构化错误，不立即删除 connect_inner
- `server/egress.rs`、`client/app.rs` 改用 Connector

**Validation**: upstream H2-only、H1-only、H2c-only、H1 cleartext 场景都能走通；`prefer_h1` 命中/miss/TTL 过期分别验证；不可重放 body 不自动 fallback；gRPC/WS 基准无回归。
...
struct CachedSender {
    selected: Arc<SelectedConnection>,   // TODO-70 后使用 Arc
...
## [TODO-69] h2c per-route sticky cache 并发化 + failover

**Priority**: Medium | **Status**: TODO
**依赖与影响**:
- 依赖：TODO-65（结构化错误）、TODO-70（`Arc<SelectedConnection>`）
- 被依赖：—
...
- cached value 当前只保存 `quinn::Connection`，无法携带 `inflight`、`conn_id`、错误分类上下文；TODO-70 后应保存 `Arc<SelectedConnection>`。
...
- `sender_cache: Arc<Mutex<HashMap<...>>>` 在并发 H2 stream 下会形成短临界区；现在锁内没有 await，问题不致命，但可以用 `DashMap` / per-route rebuild lock 把竞争范围压小。
...
- `server/registry.rs`（TODO-70 后返回 `Arc<SelectedConnection>`）
- `server/plugins/h2c/state.rs`（可选）

**Validation**: 单 h2c connection 并发请求 2 个 Host，确认 route 不串；kill cached client 后下一请求重选；`select_healthy` 只在 route 首次或失效后调用；错误 metrics 带 type/source。

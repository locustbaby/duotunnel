...
### [TODO-CR-NEW-B] VhostRouter 返回 RouteTarget 结构体
**Priority**: Medium | **Status**: TODO
**Problem**: `VhostRouter` 返回值是匿名元组 `(Arc<str>, Arc<str>)`，靠位置区分 group_id 和 proxy_name，易混淆。
**Fix**: 改为 `RouteTarget { group_id: Arc<str>, proxy_name: Arc<str> }`。

### [TODO-CR-NEW-E] 提取 ThreadLocalBuf 共享工具
**Priority**: Low | **Status**: TODO
**Problem**: `entry.rs` 和 `proxy/core.rs` 里的 unsafe peek buf 模式重复。
**Fix**: 提取到 `infra` 或 `transport` 里的共享 `ThreadLocalBuf` 工具。

### [TODO-CR4] 解耦观测性逻辑
**Priority**: Low | **Status**: TODO
**Problem**: 业务热路径里散布大量显式 `metrics::xxx()` 调用，冲突且性能受限（锁竞争）。
**Fix**: 业务层只抛 `tracing` 事件，由独立 subscriber 通过非阻塞 channel 汇总成 Prometheus metrics。

### [TODO-CR-NEW-C] 合并 TcpPeer 和 TlsTcpPeer
**Priority**: Medium | **Status**: TODO
**Problem**: 两者逻辑高度相似，仅在 TLS 握手处分叉。
**Fix**: 合并为 `TcpPeer { addr, tls: Option<TlsConfig> }`。

### [TODO-CR5] 配置流式化 (ConfigStream)
**Priority**: Low | **Status**: TODO
**Problem**: 当前是 Pull 模型，热更新依赖触发。
**Fix**: 演进为 `Stream<Item = RoutingSnapshot>` 模型，解耦配置源与业务。
...
### [TODO-62] Per-peer 协议自适应（参考 Pingora set_http_version(2,1) 模式）

**背景**：当前 `UpstreamResolver` 实现（`client/app.rs`、`server/egress.rs`）把 upstream 协议硬绑定在 config scheme 上（`http://` → H1，`https://` → H2）。downstream 进来是 H2 就用 H2Peer，是 H1 就用 HttpPeer，两者完全耦合。

**Pingora 的做法**：`set_http_version(2, 1)` = H2 优先、H1 fallback。
- 首次连接做 TLS ALPN 协商，探测 backend 实际能力
- 协商结果 H1 → `prefer_h1(peer)` 写入 per-peer 状态，后续不再尝试 H2
- H2 pool 有可复用连接 → 直接复用，无新 handshake
- **downstream 协议与 upstream 协议完全解耦**

**duotunnel 的问题**：
1. downstream H2 → upstream 必须 H2Peer，backend 不支持 H2 时直接 502
2. downstream H1 → upstream 必须 HttpPeer，浪费 backend H2 能力
3. upstream 协议由 config scheme 静态决定，没有运行时探测/记忆

**改进方向**：
- `UpstreamResolver` 不再根据 `context.protocol` 选 peer，而是根据 backend 地址的**已知能力**选
- 引入 per-upstream-addr 的协议偏好表（`DashMap<String, PreferredProto>`），初始 Unknown
- 首次连接：尝试 H2（TLS ALPN），成功 → 记录 H2，失败 → 记录 H1
- 后续连接直接查表，命中则跳过协商
- H2c（cleartext H2）backend 需 config 显式标注（无协商机制）

**文件**：`client/app.rs`、`server/egress.rs`、新增 `tunnel-lib/src/proxy/upstream_probe.rs`
**依赖**：需要 hyper client 支持连接级 ALPN 结果回调，或改用 TCP+rustls 直连后检查协商结果

> **Pingora-inspired 重构（设计 + Task）见 `docs/pingora-tasks.md`（TODO-63..72）**

...
`tunnel-store` is the persistence layer for DuoTunnel. It provides SQLite-backed implementations for routing rules (`RuleStore`) and authentication (`AuthStore`). Configuration parsing for server bootstrap is also housed here.
...
These preserve existing call sites in `server` and `tunnel-service`. New code should use canonical paths.
...
- `ClientStatus`, `TokenStatus` (re-exported from `tunnel_lib`)
...
- Config parsing is behind `server-config` feature; `tunnel-service` and `server` enable this

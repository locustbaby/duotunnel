...
`duotunnel-core` is the shared library crate for DuoTunnel. It provides protocol primitives, transport adapters, plugin infrastructure, and domain types used by `duotunnel-server`, `duotunnel-client`, and `duotunnel-ctld`.

## Module Layout

```
crates/duotunnel-core/src/

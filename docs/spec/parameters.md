...
| **peek_buf_size** | `PeekBufPool::new(size)`：client entry (`duotunnel-client/egress/listener.rs:107`)、server ingress TCP handler (`duotunnel-server/bootstrap/mod.rs:322` → `duotunnel-server/ingress/handlers/tcp.rs:54`)、`ProxyEngine` QUIC stream sniff (`duotunnel-lib/src/proxy/core.rs`) | 16 KiB | `proxy_buffers.peek_buf_size` | 缓冲区不足会导致协议识别失败 (Protocol::Unknown) | 日志: `detected protocol: Unknown`; 默认值见 `config/proxy_buffers.rs:18`。Peek buffer pool 始终启用并按线程保留有界空闲 buffer。`IngressDispatcher` 的 Phase 1 仍使用独立的协议上限 `SNIFF_LIMIT = 4096`，用于限制入口 sniff，不属于 ProxyEngine 的可调 buffer。 |
| **http_header_buf_size** | `protocol/driver/h1.rs::Http1Driver::new_with_buffer_params`、`egress/http.rs::forward_http_with_buffer_params` | 8 KiB (`config/proxy_buffers.rs:19`) | `proxy_buffers.http_header_buf_size` | 控制 H1 parser 初始容量、单次 QUIC header read 和允许的最大 header buffer；有效范围由统一 buffer validation 限制到 1–64 KiB。H2 路径不消费此参数，由 H2 framing/header 设置控制 | 检查启动配置和 H1 buffer focused test |
| **sniff_timeout_ms** | client entry `duotunnel-client/egress/listener.rs:109`；server ingress `plugin/dispatcher.rs:38`（HTTP 入口经 `handlers/http.rs:47`）/ `handlers/tcp.rs:60`，均取 `state.sniff_timeout()` (`duotunnel-server/bootstrap/mod.rs:229`) | 2500ms | `proxy_buffers.sniff_timeout_ms` | 协议识别的最大时间预算；超时会主动断开以防御 Slowloris 攻击 | 日志: `protocol sniffing timed out`。`ProxyEngine::run_stream` 的 QUIC 流 sniff 仍使用独立的 5s 协议超时，当前不与入口 Slowloris timeout 混用。 |
...
| **Reverse stream admission** | server `handlers/quic.rs` + `tunnel_handler.rs` | 每连接 1000；进程 4096；RoutingInfo 8 KiB / 5s | ❌ (常量) | permit 满立即 reset/stop；routing 首帧超大或超时关闭该 stream，避免认证连接堆积 stalled task/allocation | `duotunnel_reverse_stream_rejected_total`、`duotunnel_reverse_streams_active` |
...
| 参数 (Parameter) | 消费者 / 逻辑位置 (Consumer / Used by) | 默认值 / 其他值 | YAML 路径 | 阈值影响 (Impact) | 排查手段 (Debugging / Logs) |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **relay_buf_size** | `engine/copy.rs` 的池化 `BytesMut`，通过 `bridge::*_with_buffer_size`、`engine/relay::*_with_buffer_size`、client entry、server ingress 和 server egress 调用 | 64 KiB / 范围 >=4K（`normalize_relay_buf_size` 向上夹到 `MIN_RELAY_BUF_SIZE`） | `proxy_buffers.relay_buf_size` | **内存**: buffer 走 thread-local + 全局两级有界池；配置现在覆盖 QUIC↔TCP、generic TCP/TLS 和 ingress/egress relay。它大于 8 KiB 的 H1 body chunk 是有意的：relay 以降低 syscall 密度为目标，H1 parser 以请求粒度为目标 | `top/htop` 观察 RSS；focused relay capacity test；默认值见 `proxy/buffer_params.rs` |
| **http_body_chunk** | `protocol/driver/h1.rs::Http1Driver` 和 `egress/http.rs::forward_http_with_buffer_params` 的 QUIC body `read_chunk` | 8 KiB (`config/proxy_buffers.rs:20`) | `proxy_buffers.http_body_chunk_size` | 控制 H1 request body 的单次读取 chunk；默认仍为 8 KiB。H2 路径不消费此参数，由 H2 framing 控制 body delivery | 检查 H1 parser focused test和 HTTP body read path |
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| **Routing Selection** | `ClientGroup::select_healthy` (`duotunnel-server/ingress/registry.rs`) | 按轮转起点扫描所有 shard；每个 shard 选出本 shard 的最低 inflight 候选，再在所有 shard winner 中选全局最低值；同负载时按轮转顺序打散 | shard 数 (`quic.shards`) | 选择成本为 O(shard 数)，不随分组内 Client 连接数线性增长；相比只优先固定 shard，能够把请求溢出到其它空闲 shard，降低多 shard 场景的尾延迟 | `select_across_shards` 的动态负载公平性测试；生产前仍需 T5 基线下的跨 shard benchmark |
...
- 已完成死配置清理：`http_header_buf_size`、`http_body_chunk_size` 和 client/server `relay_buf_size` 均有实际消费者；后续只需用 T5 基线评估不同值的性能收益，不应据此改变默认值

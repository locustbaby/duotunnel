...
```
duotunnel-client/runtime/app.rs:38   build_quic_endpoint()  ← 全进程唯一一次
        │
        └─ TunnelPoolService { endpoint }        （duotunnel-client/tunnel/mod.rs:17）
              │
...
| 事实 | 证据 | 对设计的意义 |
| --- | --- | --- |
| endpoint 全进程建一次 | `duotunnel-client/runtime/app.rs:38` | 唯一改动源头 |
| N 个 slot 共享同一 clone | `duotunnel-client/tunnel/pool.rs:23` | **改这里即可** |
...
| UDP socket 绑 `0.0.0.0:0`（内核分配临时端口） | `duotunnel-client/tunnel/endpoint.rs:24` | 多 endpoint 天然不冲突 |
| `EntryConnPool` **不感知** endpoint，只存 `ConnectionHandle` | `duotunnel-client/tunnel/conn_pool.rs` 全文无 endpoint | **pool 不用改** |
...
```rust
// duotunnel-client/tunnel/mod.rs —— TunnelPoolService 改为持有 N 个 endpoint
...
// duotunnel-client/tunnel/pool.rs —— coordinator 保留 owner；每个 connection slot 取稳定映射的 clone
...
// duotunnel-client/runtime/app.rs —— 建 N 个而非 1 个

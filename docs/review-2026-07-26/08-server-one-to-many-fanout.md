...
| F2 | registry inflight slot 表硬编码 4096，全 server 存活 client 连接上限 4096，超出后鉴权成功但注册失败 | **低-中**（按实际规模 2-3 duotunnel-client/group，4096 远非约束；属可观测性卫生项） | ❌ 不可见 | TODO-146 |
...
| 客户端侧同名原语 | `duotunnel-client/tunnel/conn_pool.rs` 的 `next_conn_for_shard_excluding` 也用 `pick_from_preferred_shards`，但**语义不同且可接受**：那里是"在 client 自己的多条隧道连接间为某条流选一条"，preferred=hash(host,"entry") 是把流散布到不同本地连接的亲和策略，且其调用点带 `excluding` 重试循环会在失败时遍历所有连接（`egress/listener.rs:165-169`）。故 client 侧**不需要**同样改动——本缺陷是 server 特有 |
...
**明确不做**：**不实现可增长表 / 分段表**（TODO-146 原选项 (c)）。在 2-3 duotunnel-client/group 的
...
- **F1 是前置**：任何"绑核/多核化"（02）落地前必须先修 F1，否则多核化会立刻
  放大分发失衡，且压测结论不可信。
- **F2 与 F1 独立且不阻塞多核化**：按 2-3 duotunnel-client/group 的实际规模，4096 不构成约束，

...
（`duotunnel-lib/src/config/file.rs:228-232`）——一个 `mode: tcp` 端口**静态绑定唯一一组
...
- **完整解析/重编码**（终止侧）：`Http1Driver`（`driver/h1.rs`）keep-alive 循环，
  在做 L7 的一侧（ingress=duotunnel-client/egress=server）完整解析请求、构造、重编码响应。
...
`IngressModeDef` 只有 `Http` 与 `Tcp` 两个变体（`duotunnel-lib/src/config/file.rs:217-220`），
配置层根本无法声明一个 UDP 入口。
...
- **server**：`UdpSessionManager`（`handlers/udp_datagram.rs`）每 QUIC 连接一个；
  ...
  扫描，`:47-72`）。
- **上游选择**：`resolve_udp_target`→`next_healthy` 轮询（`duotunnel-server/egress/mod.rs:43`）。

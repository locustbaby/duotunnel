...
与 ingress **完全对称**：client 侧是轻的字节转发（`client/egress/listener.rs:95-253`
每连接 sniff → allowlist → P2C 选连接 → `open_stream` → `relay_quic_to_tcp`），
**每请求的 L7 全解析落在 server 侧**（`tunnel_handler.rs:6-29` → `ProxyEngine` →
`ServerEgressMap::upstream_peer`（`server/egress/mod.rs:84-152`，每 stream 一次 vhost 查找 +
...
> 注：`tunnel-lib/src/egress/http.rs:91 forward_http` 并不在这条链路上——它已是
...
**现象与证据**：`tunnel-lib/src/engine/copy.rs:17-44` `take_buffer` 三条路径（thread-local
...
落点：`IngressClientApp::upstream_peer`（`client/ingress/app.rs:104`）读取该 upstream 的 `mode`——
`l4` 直接返回 `PeerSpec::Tcp`（**忽略 sniff 出的 protocol**），`l7` 维持现有行为；egress 侧
`ServerEgressMap::upstream_peer`（`server/egress/mod.rs:84`）同构。
...
- 慢路径参数默认 yield=80%、sleep=95%（tunnel-store config/mod.rs:98-103）：

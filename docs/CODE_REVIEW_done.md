...
## 一、 数据流与协议层分析 (已完成项参见 donelist.md)

- **Relay 层归一化 (CR2)**: 已统一为 64KB `copy_buf`，消除了逻辑重复。
- **协议检测枚举化 (CR1)**: 已实现 `Protocol` 枚举，消除了热路径上的字符串开销。
...
**改进方向** (CR-NEW-A): 已通过 `recv_typed_message<T>` 封装两步消息读取。
...
**改进方向** (CR-NEW-D): 已完成 API 分组与可见性清理，底层函数已收入 `pub mod relay`。

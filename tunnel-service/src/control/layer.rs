...
use tracing::warn;
use tunnel_lib::ctld_proto::ConfigOperation;
use tunnel_store::rules::{RoutingData, RuleStore};
...
async fn load_yaml(path: &Path) -> Result<ConfigLayer> {
    ...
    let config = tunnel_store::server_config::RoutingConfigFile::load(
        path.to_str()
            .ok_or_else(|| anyhow::anyhow!("invalid YAML path"))?,
    )?;
    Ok(ConfigLayer {
        source_revision,
        routing: tunnel_store::server_config::routing_data_from_routing_config(&config),
    })
}
...
mod tests {
    use super::*;
    use tunnel_store::rules::{IngressListener, IngressListenerMode};

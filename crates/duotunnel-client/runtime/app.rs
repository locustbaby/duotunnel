...
async fn run_client_process(bootstrap: ClientBootstrap) -> Result<()> {
    ...
    let resolved_connections = duotunnel_core::resolve_connection_count(config.quic.connections);
    ...
    let shard_count = duotunnel_core::resolve_shard_count(
        config.quic.shards,
        Some(resolved_connections as usize),
    );
    let accept_workers = duotunnel_core::resolve_accept_workers(config.entry.accept_workers);
    info!(
    ...
    if let Some(entry_port) = config.entry.port {
        let entry_tcp_params = duotunnel_core::TcpParams::from(&config.tcp);
...
async fn wait_for_shutdown_drain(role: &'static str) {
    ...
    let active = duotunnel_core::METRICS.active_connections();
    let pending = duotunnel_core::METRICS.pending_streams();
    if drained {
        info!(
            role,

use anyhow::Result;
use duotunnel_core::config::{HttpPoolConfig, ProxyBufferConfig, TcpConfig};
use duotunnel_core::transport::quic::QuicTransportParams;
use figment::{
...
impl From<OverloadMode> for duotunnel_core::SharedOverloadMode {
    fn from(m: OverloadMode) -> Self {
        match m {
            OverloadMode::InflightSlowpath => duotunnel_core::SharedOverloadMode::InflightSlowpath,
            OverloadMode::Burst => duotunnel_core::SharedOverloadMode::Burst,
        }
    }
}
...
impl From<BackoffStrategy> for duotunnel_core::BackoffStrategy {
    fn from(s: BackoffStrategy) -> Self {
        match s {
            BackoffStrategy::None => duotunnel_core::BackoffStrategy::None,
            BackoffStrategy::Fixed => duotunnel_core::BackoffStrategy::Fixed,
            BackoffStrategy::Exponential => duotunnel_core::BackoffStrategy::Exponential,
        }
    }
}
...
impl OverloadConfig {
    pub fn resolve(&self, max_concurrent_streams: u32) -> duotunnel_core::OverloadLimits {
        duotunnel_core::OverloadLimits::resolve(
...
impl ClientConfigFile {
    pub fn load(path: &str) -> Result<Self> {
        let resolved = duotunnel_core::resolve_config_path(path)?;
    ...
    fn validate(&self) -> Result<()> {
        ...
        if self.proxy_buffers.relay_buf_size
            < duotunnel_core::proxy::buffer_params::MIN_RELAY_BUF_SIZE
        {
            errors.push(format!(
...
mod tests {
    ...
    fn test_overload_config_resolve_absolute() {
        ...
        assert_eq!(limits.mode, duotunnel_core::SharedOverloadMode::Burst);
        assert_eq!(limits.inflight_yield_threshold, 100);
        assert_eq!(limits.inflight_sleep_threshold, 200);
        assert_eq!(limits.backoff, duotunnel_core::BackoffStrategy::Fixed);
        assert_eq!(
    ...
    fn test_overload_config_resolve_percentage() {
        ...
        assert_eq!(
            limits.mode,
            duotunnel_core::SharedOverloadMode::InflightSlowpath
        );
        assert_eq!(limits.inflight_yield_threshold, 500); // 1000 * 0.5
        assert_eq!(limits.inflight_sleep_threshold, 800); // 1000 * 0.8
        assert_eq!(limits.backoff, duotunnel_core::BackoffStrategy::Exponential);
        assert_eq!(
            limits.inflight_sleep_budget,

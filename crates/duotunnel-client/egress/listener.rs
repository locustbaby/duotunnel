...
pub async fn start_entry_listener(
    ...
    for _ in 0..accept_workers {
        listeners.push(Arc::new(duotunnel_core::build_reuseport_listener(addr)?));
    }
...
async fn handle_entry_connection(
    ...
    let (protocol, host, initial_bytes) = if sniffed.bytes_read > 0 {
        ...
        (duotunnel_core::proxy::core::Protocol::Unknown, None, None)
    };
    ...
    if let Some(ref host_raw) = host {
        if pool.is_rejected_host(host_raw) {
            ...
            if protocol == duotunnel_core::proxy::core::Protocol::H1 {
...
mod tests {
    use super::*;
    use duotunnel_core::EgressVhostRuleDef;
    ...
    async fn test_handle_entry_connection_http_reject() {
        ...
        let overload = OverloadLimits::resolve(
            duotunnel_core::SharedOverloadMode::Burst,
            100,
            0,
            0,
            None,
            ...
            Some(0.0),
            0,
            duotunnel_core::BackoffStrategy::Exponential,
        );

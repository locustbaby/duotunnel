...
use duotunnel_core::QuicTransportParams;
...
pub(crate) async fn build_quic_endpoint(config: &ClientConfigFile) -> Result<quinn::Endpoint> {
    ...
    crypto.alpn_protocols = vec![duotunnel_core::TUNNEL_ALPN.to_vec()];
    let mut client_config = quinn::ClientConfig::new(Arc::new(
        quinn::crypto::rustls::QuicClientConfig::try_from(crypto)?,
    ));
    let quic_params = QuicTransportParams::from(&config.quic);
    let transport_config = duotunnel_core::build_transport_config(&quic_params)?;
    ...
    let udp_socket = duotunnel_core::build_udp_socket("0.0.0.0:0".parse()?, &quic_params)?;
...
mod tests {
    ...
    fn native_root_loader_keeps_valid_roots_when_some_loads_fail() {
        let (certs, _) =
            duotunnel_core::infra::pki::generate_self_signed_cert_for_host("example.com").unwrap();

...
use bytes::Bytes;
use duotunnel_core::plugin::{LoadBalancer, PickCtx, Resolver, Target};
use duotunnel_core::proxy::core::{Context as ProxyContext, Protocol, UpstreamResolver};
use duotunnel_core::proxy::http_connector::SharedHttpConnector;
use duotunnel_core::proxy::peers::{
    BasicPeerSpec, HttpPeerSpec, MitmPeerSpec, PeerSpec, TlsPeerSpec,
};
use duotunnel_core::proxy::tcp::UpstreamScheme;
use duotunnel_core::{ClientConfig, HttpClientParams, ProxyError};
...
impl<L: LoadBalancer, R: Resolver> LocalProxyMap<L, R> {
        ...
        for upstream in &config.upstreams {
            ...
            let targets: Vec<Target> = raw
                ...
                .map(|addr| {
                    ...
                    let parsed = duotunnel_core::transport::addr::parse_upstream(addr);
                    Target {
                        host: parsed.host,
                        port: parsed.port,
                        scheme,
    ...
    pub async fn resolve_addr(&self, connect_addr_str: &str) -> Result<SocketAddr> {
        ...
        let parsed = duotunnel_core::transport::addr::parse_upstream(connect_addr_str);
...
impl<L, R> IngressClientApp<L, R> {
    pub fn new(map: Arc<LocalProxyMap<L, R>>, tcp_params: duotunnel_core::TcpParams) -> Self {
        Self { map, tcp_params }
    }
}
    ...
    async fn connect_peer(
        ...
        match peer {
            ...
            PeerSpec::MitmH2(spec) => {
                let server_config = duotunnel_core::get_or_create_server_config(&spec.tls_host)
                    .await
                    .map_err(|e| ProxyError::upstream_connect(e.to_string()))?;
                let acceptor = tokio_rustls::TlsAcceptor::from(server_config);
                let stream = duotunnel_core::QuinnStream { send, recv };
                let stream = if let Some(init) = initial_data {
                    duotunnel_core::PrefixedReadWrite::new(stream, init)
                } else {
                    duotunnel_core::PrefixedReadWrite::new(stream, bytes::Bytes::new())
                };

...
use duotunnel_store::rules::RoutingData;
use duotunnel_store::{AuthStore, RuleStore, TokenListEntry};
...
impl ControlService {
    ...
    ) -> duotunnel_core::ctld_proto::ControlRevision {
        duotunnel_core::ctld_proto::ControlRevision {
    ...
    async fn build_and_commit_snapshot(
        ...
        snapshot_pool: Option<&sqlx::SqlitePool>,
        version: u64,
    ) -> Result<(ConfigSnapshot, duotunnel_core::ctld_proto::ControlRevision)> {
        if let Some(pool) = snapshot_pool {
            let mut tx = pool.begin().await?;
            let routing =
                duotunnel_store::sqlite_rules::SqliteRuleStore::load_routing_on(&mut tx).await?;
            ...
            let content_hash = duotunnel_core::ctld_proto::snapshot_content_hash(&snapshot)?;
            let revision =
    ...
    pub async fn apply_config_override(
        &self,
        operation: &duotunnel_core::ctld_proto::ConfigOperation,
    ) -> Result<()> {
        let _guard = self.config_mutation_lock.lock().await;
        let Some(pool) = self.snapshot_pool.as_ref() else {
            anyhow::bail!("layered configuration requires SQLite");
...
mod tests {
    ...
    use duotunnel_store::sqlite::{open_sqlite_pool, SqliteAuthStore};
    use duotunnel_store::sqlite_rules::SqliteRuleStore;
    use duotunnel_store::TokenStatus;

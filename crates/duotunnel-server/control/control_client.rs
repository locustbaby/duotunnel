...
use duotunnel_core::models::msg::{send_message, MessageType};
...
async fn handle_delta<W>(
    delta: duotunnel_core::ctld_proto::ConfigDelta,
    state: &Arc<ServerState>,
    watch_state: &mut WatchState,
    snapshot_path: &Path,
    writer: &mut W,
...
fn build_token_map(
    entries: &[duotunnel_core::shared::TokenCacheEntryDef],
...
fn token_is_active(entry: &CacheEntry) -> bool {
    entry.client_status == duotunnel_store::ClientStatus::Active
        && entry.token_status == duotunnel_store::TokenStatus::Active
}
...
// ── Type conversions: duotunnel_store routing types → server config types ────────

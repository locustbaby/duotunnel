...
use duotunnel_core::{HttpClientParams, RouteTarget, VhostRouter};
use duotunnel_store::{AuthResult, AuthStore};
...
impl ServerBootstrap {
    ...
    pub(crate) fn pki(&self) -> &duotunnel_core::PkiParams {
...
pub(crate) struct RoutingSnapshot {
    ...
    egress_rules: Vec<duotunnel_core::EgressVhostRuleDef>,
}
...
impl RoutingSnapshot {
    ...
    pub(crate) fn client_config_for_group(
        &self,
        group_id: &str,
    ) -> Option<duotunnel_core::ClientConfig> {
    ...
    pub(crate) fn tcp_route(
        &self,
        listener_port: u16,
    ) -> Option<(duotunnel_core::GroupId, duotunnel_core::ProxyName)> {
        self.tunnel_management
...
struct IngressRuntime {
    ...
    proxy_buffer_params: duotunnel_core::ProxyBufferParams,
    peek_buf_pool: crate::PeekBufPool,
    generation: Arc<ArcSwap<RuntimeGeneration>>,
    upstream_health: Arc<duotunnel_core::proxy::upstream::UpstreamHealthRegistry>,
    listeners: listener_mgr::ListenerManager,
    overload_limits: duotunnel_core::OverloadLimits,
    plugin_registry: Arc<duotunnel_core::plugin::PluginRegistry>,
...
impl ServerState {
    ...
    pub(crate) fn upstream_health(
        &self,
    ) -> Arc<duotunnel_core::proxy::upstream::UpstreamHealthRegistry> {
        self.ingress.upstream_health.clone()
    }

    pub(crate) fn accept_workers(&self) -> usize {
        duotunnel_core::resolve_accept_workers(self.ingress.config.server.accept_workers)
    }
    ...
    pub(crate) fn quic_transport_params(&self) -> duotunnel_core::QuicTransportParams {
        duotunnel_core::QuicTransportParams::from(&self.ingress.config.server.quic)
    }
    ...
    pub(crate) async fn authenticate_pinned(
        &self,
        raw_token: &str,
    ) -> std::result::Result<(AuthResult, Arc<RuntimeGeneration>), duotunnel_store::AuthError> {
        let generation = self.runtime_generation();
    ...
    pub(crate) fn tcp_params(&self) -> &duotunnel_core::TcpParams {
    ...
    pub(crate) fn overload_limits(&self) -> &duotunnel_core::OverloadLimits {
        &self.ingress.overload_limits
    }

    pub(crate) fn plugin_registry(&self) -> &Arc<duotunnel_core::plugin::PluginRegistry> {
...
pub(crate) async fn build_server_state(bootstrap: &ServerBootstrap) -> Result<Arc<ServerState>> {
    ...
    let upstream_health =
        Arc::new(duotunnel_core::proxy::upstream::UpstreamHealthRegistry::default());
    let initial_snapshot =
        build_routing_snapshot_with_health(&tm, &egress, &http_params, upstream_health.clone())?;
    let proxy_buffer_params =
        duotunnel_core::ProxyBufferParams::from(&bootstrap.config.server.proxy_buffers);
    let peek_buf_pool = crate::PeekBufPool::new(proxy_buffer_params.peek_buf_size);
    let max_streams = duotunnel_core::QuicTransportParams::from(&bootstrap.config.server.quic)
    ...
    let shard_count =
        duotunnel_core::resolve_shard_count(bootstrap.config.server.quic.shards, None);
    let accept_workers =
        duotunnel_core::resolve_accept_workers(bootstrap.config.server.accept_workers);
    info!(
        shards = shard_count,
        accept_workers = accept_workers,
        configured_worker_threads = duotunnel_core::configured_worker_threads(),
        cpu_parallelism = duotunnel_core::available_parallelism(),
        cgroup_cpu_limit = ?duotunnel_core::cgroup_cpu_limit(),
        effective_parallelism = duotunnel_core::effective_runtime_parallelism(),
    ...
    let plugin_registry = {
        use duotunnel_core::plugin::PluginRegistry;

        let mut reg = PluginRegistry::new();
        let route_resolver: Arc<dyn duotunnel_core::plugin::RouteResolver> =
    ...
    Ok(Arc::new(ServerState {
        ingress: IngressRuntime {
            ...
            tcp_params: duotunnel_core::TcpParams::from(&bootstrap.config.server.tcp),
            proxy_buffer_params,
            peek_buf_pool,
            generation,
            upstream_health,
...
pub(crate) fn build_routing_snapshot_with_health(
    ...
    let egress_rules = egress
        ...
        .vhost
        .iter()
        .map(|r| duotunnel_core::EgressVhostRuleDef {

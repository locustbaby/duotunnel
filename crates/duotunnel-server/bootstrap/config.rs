// Re-export the canonical config types from duotunnel-store so the rest of the
// server crate can keep importing from `crate::config`.
pub use duotunnel_store::server_config::{
...
pub fn build_client_config_for_group(
    tm: &TunnelManagement,
    egress_rules: &[duotunnel_core::EgressVhostRuleDef],
    group_id: &str,
    ...
    let upstreams = group
        .upstreams
        .iter()
        .map(|(name, def)| duotunnel_core::UpstreamConfig {
            name: name.clone(),
            servers: def
                .servers
                .iter()
                .map(|s| duotunnel_core::UpstreamServer {
        ...
        .collect();
    Some(duotunnel_core::ClientConfig {

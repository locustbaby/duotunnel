...
use duotunnel_core::{EmptyBodyRetryTemplate, OpenStreamRequest, ProxyError, RouteTarget};
...
impl IngressProtocolHandler for TlsHandler {
    ...
    async fn handle(
        ...
        let server_config = duotunnel_core::infra::pki::get_or_create_server_config(&host)
            .await
        ...
        if negotiated_h2 {
            ...
            let connection = duotunnel_core::proxy::h2::hardened_h1_server_builder()
                .serve_connection(io, service);
            tokio::pin!(connection);
            tokio::select! {
                result = &mut connection => {

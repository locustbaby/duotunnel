...
use duotunnel_core::{ErrorKind, OpenStreamRequest, ProxyError};
...
impl IngressProtocolHandler for H1Handler {
    ...
    async fn handle(
        ...
        let protocol = match hint.protocol {
            duotunnel_core::proxy::core::Protocol::WebSocket => {
                duotunnel_core::proxy::core::Protocol::WebSocket
            }
            _ => duotunnel_core::proxy::core::Protocol::H1,
        };
        ...
        let opened = loop {
            ...
            duotunnel_core::maybe_slow_path(selected.handle.connection_state(), &ctx.overload)
                .await;
        ...
        duotunnel_core::proxy::forward_prefixed_to_client(send, recv, stream, ctx.relay_buf_size)
            .await
    }
}

...
use duotunnel_core::plugin::{IngressProtocolHandler, ProtocolKind, Route, ServerCtx};
use duotunnel_core::transport::listener::RouteTarget;
use duotunnel_core::{EmptyBodyRetryTemplate, OpenStreamRequest, ProxyError};
...
struct CachedSender {
    ...
    sender: duotunnel_core::H2Sender,
}

fn error_response(err: &ProxyError) -> Response<duotunnel_core::proxy::h2_proxy::BoxBody> {
...
fn observe_h2c_error(metrics: &Arc<dyn duotunnel_core::plugin::MetricsSink>, err: &ProxyError) {
    duotunnel_core::plugin::observe_proxy_error(
        metrics.as_ref(),
        ProtocolKind::H2c.as_label(),
        err,
    );
    metrics.incr(
...
fn get_or_create_sender(
    ...
    let entry = CachedSender {
        selected,
        sender: duotunnel_core::new_h2_sender(),
    };
...
impl IngressProtocolHandler for H2cHandler {
    ...
    async fn handle(
        &self,
        stream: duotunnel_core::PrefixedReadWrite<tokio::net::TcpStream>,
        _route: Option<Route>,
        ctx: &ServerCtx,
    ) -> Result<()> {
        ...
        let service = service_fn(move |req: Request<hyper::body::Incoming>| {
            ...
            async move {
                let _tracked =
                    duotunnel_core::track_resource(duotunnel_core::TrackedResource::HttpRequest);
                ...
                let routing_info = duotunnel_core::RoutingInfo {
                    proxy_name: proxy_name.clone(),
                    src_addr,
                    src_port,
                    protocol: duotunnel_core::proxy::core::Protocol::H2,
                    host: Some(host),
                };
                ...
                match duotunnel_core::forward_h2_request(
        ...
        let connection = duotunnel_core::proxy::h2::hardened_h2_server_builder(

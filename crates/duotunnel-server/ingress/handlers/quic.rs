...
pub async fn run_quic_server(state: Arc<ServerState>, shutdown: CancellationToken) -> Result<()> {
    ...
    let server_config = duotunnel_core::transport::quic::create_server_config_with(&quic_params)?;
    let udp_socket = duotunnel_core::build_udp_socket(addr, &quic_params)?;
...
async fn handle_quic_connection(
    ...
    let auth_result = match auth_outcome {
        ...
        Err(e) => {
            ...
            let resp = match e {
                duotunnel_store::AuthError::Internal(_) => {
                    LoginResp::failure_retryable("authentication unavailable")
                }

...
fn validate_response(
    ...
    if !ok_status {
        return Err(format!(
            "{proto}: unexpected status {status}, body: {}",
    ...
    if status != 200 {
        return Ok(format!(
            "{proto} {status} (allowed status, len={})",
            body.len()
        ));
    }
    ...
    if let Some(expected) = expect_body {
        if !body.contains(expected) {
            return Err(format!(
                "{proto}: expected body to contain {:?}, got: {}",
                expected,
    ...
    match serde_json::from_str::<serde_json::Value>(body) {
        ...
        Ok(_) => Ok(format!(
            "{proto} {status} (non-echo response, len={})",
            body.len()
        )),
    ...
    let echo = tokio::time::timeout(Duration::from_secs(10), async {
        while let Some(msg) = ws.next().await {
            match msg {
                ...
                Ok(Message::Close(_)) => {
                    return Err("server closed connection before echo".to_string())
                }
                Ok(_) => {}
...
async fn run_grpc(addr: &str, args: &[String]) -> Result<String, String> {
    ...
    let resp = tokio::time::timeout(Duration::from_secs(15), client.check(request))
        .await
        .map_err(|_| "gRPC Health/Check timeout".to_string())?
        .map_err(|e| {
            format!(
                "gRPC Health/Check error: status={} message={}",
                e.code(),
                e.message()
            )
        })?;
    ...
    if status == 1 {
        ...
        Err(format!(
            "Health/Check returned non-SERVING status: {status}"
        ))
    }
}
...
async fn run_grpc_echo(addr: &str, args: &[String]) -> Result<String, String> {
    ...
    let resp = tokio::time::timeout(Duration::from_secs(15), grpc.unary(request, path, codec))
        ...
        .map_err(|e| {
            format!(
                "gRPC Echo error: status={} message={}",
                e.code(),
                e.message()
            )
        })?;

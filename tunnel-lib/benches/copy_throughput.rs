...
use tokio::io::{AsyncReadExt, repeat, sink};
use tunnel_lib::engine::copy::{copy_buffered_then_shutdown, copy_buffered_then_finish};
use std::sync::Arc;
...
fn bench_copy_throughput(c: &mut Criterion) {
    ...
    let (client_conn, server_conn, _ce, _se) = rt.block_on(async {
        let (certs, key) = tunnel_lib::infra::pki::generate_self_signed_cert().unwrap();
        ...
        let connecting = client_endpoint.connect_with(client_config, server_addr, "localhost").unwrap();
    ...
    for &buf_size in &buf_sizes {
        ...
        group.bench_with_input(
            ...
            |b, &buf_size| {
                let cc = c_conn.clone();
                let sc = s_conn.clone();
                b.to_async(&rt).iter(|| run_quic_finish_bench(cc.clone(), sc.clone(), data_size, buf_size));
            },
        );
    }

...
impl ClientRulesEngine {
    ...
    fn update_rules(&mut self, rules: Vec<Rule>, upstreams: Vec<tunnel_lib::tunnel::Upstream>) {
        ...
        info!("Updated local rules: {} HTTP, {} gRPC, {} upstreams", 
                self.http_rules.len(), self.grpc_rules.len(), self.upstreams.len());
    }
...
impl TunnelClient {
    fn new(client_id: String, group_id: String, server_addr: String) -> (Self, mpsc::Receiver<TunnelMessage>) {
        ...
        match msg.payload {
            Some(tunnel_message::Payload::HttpRequest(req)) => {
                ...
                if msg.direction == Direction::ServerToClient as i32 {
                    let response = self.handle_tunnel_http_request(req, msg.request_id.clone()).await;
            ...
            Some(tunnel_message::Payload::ConfigSyncResponse(resp)) => {
                // 处理配置同步响应
                info!("Received config sync response: version {}, {} rules, {} upstreams", 
                      resp.config_version, resp.rules.len(), resp.upstreams.len());
    ...
    async fn handle_tunnel_http_request(&self, req: HttpRequest, _request_id: String) -> HttpResponse {
        let host = req.host.as_str();
        let path = req.url.split('?').next().unwrap_or("/");
        
        info!("Processing tunnel request: {} {}", req.method, req.url);
        
        // 使用本地规则引擎匹配规则
        let rules_engine = self.rules_engine.lock().unwrap();
        if let Some(rule) = rules_engine.match_http_rule(host, path) {
            // 处理 action_proxy_pass
            if !rule.action_proxy_pass.is_empty() {
                return self.forward_to_backend(&req, &rule.action_proxy_pass).await;
            }
            
            // 处理 action_upstream
            if !rule.action_upstream.is_empty() {
                if let Some(backend) = rules_engine.pick_backend(&rule.action_upstream) {
                    return self.forward_to_backend(&req, &backend).await;
                }
            }
        }
        ...
        self.forward_to_backend(&req, &local_url).await
    }

    async fn forward_to_backend(&self, req: &HttpRequest, target_url: &str) -> HttpResponse {
        ...
        if is_https {
            self.forward_to_backend_https(req, target_url).await
        } else {
            self.forward_to_backend_http(req, target_url).await
        }
    }

    async fn forward_to_backend_http(&self, req: &HttpRequest, target_url: &str) -> HttpResponse {
        use hyper::Client;
    ...
    async fn forward_to_backend_https(&self, req: &HttpRequest, target_url: &str) -> HttpResponse {
        use hyper::Client;
        use hyper_tls::HttpsConnector;
    ...
    async fn start_config_sync_via_tunnel(&self, tx: mpsc::Sender<TunnelMessage>) {
        ...
        tokio::spawn(async move {
            ...
            if let Err(e) = tx.send(config_sync_msg).await {
                error!("Failed to send initial config sync: {}", e);
            }
...
async fn main() -> anyhow::Result<()> {
    ...
    let (client, _rx) = TunnelClient::new(client_id, group_id, server_addr.clone());
                ...
                Ok::<_, hyper::Error>(service_fn(move |req| {
                    ...
                    async move {
                        ...
                        let tunnel_msg = tunnel_lib::tunnel::TunnelMessage {
                            client_id: "".to_string(), // client_id 由 server 识别

...
impl LocalProxyMap {
    pub fn from_config(config: &ClientConfig, http_params: &HttpClientParams) -> Self {
        ...
        Self { upstreams, https_client, h2c_client }
    }
...
impl ProxyApp for ClientApp {
    async fn upstream_peer(&self, context: &mut ProxyContext) -> Result<PeerKind> {
        ...
        let upstream_addr = self
            ...
            .ok_or_else(|| {
                anyhow::anyhow!("no upstream for proxy_name: {}", routing.proxy_name)
            })?;
        use tunnel_lib::proxy::tcp::UpstreamScheme;
        let (scheme, connect_addr_str, tls_host) = UpstreamScheme::from_address(
            &upstream_addr,
        );
        ...
        match context.protocol {
            ...
            Protocol::H2 => {
                ...
                Ok(
                    PeerKind::H2(
                        Box::new(tunnel_lib::proxy::h2::H2Peer {
                            target_host: connect_addr_str,
                            scheme,
            ...
            Protocol::WebSocket => {
                ...
                let target_addr = if let Ok(addr) = connect_addr_str
                    .parse::<std::net::SocketAddr>()
                {
                    ...
                    addrs
                        .next()
                        .ok_or_else(|| {
                            anyhow!("no resolved IP for {}", connect_addr_str)
                        })?
                };
                if is_https {
                    Ok(
                        PeerKind::Tls(
                            tunnel_lib::proxy::tcp::TlsTcpPeer::new_with_params(
                                target_addr,
                                tls_host
                                    .ok_or_else(|| anyhow!("TLS host required for WSS"))?,
                                None,
                    ...
                    Ok(
                        PeerKind::Tcp(tunnel_lib::proxy::tcp::TcpPeer {
                            target_addr,
            ...
            Protocol::Tcp => {
                ...
                if is_https {
                    ...
                    let (certs, key) = tunnel_lib::infra::pki::generate_self_signed_cert_for_host(
                        host_for_cert,
                    )?;
                    ...
                    impl MitmH2Peer {
                        async fn do_connect(
                            ...
                            let stream = tunnel_lib::QuinnStream {
                                send,
                                recv,
                            };
                            let stream = if let Some(init) = initial_data {
                                ...
                                tunnel_lib::PrefixedReadWrite::new(
                                    stream,
                                    bytes::Bytes::new(),
                                )
                            };

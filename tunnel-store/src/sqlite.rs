...
impl AuthStore for SqliteAuthStore {
    async fn authenticate(&self, raw_token: &str) -> std::result::Result<AuthResult, AuthError> {
        ...
        let row = sqlx::query(
            "SELECT c.name as client_name, c.status as client_status, t.status as token_status
        ...
        if token_status == "revoked" {
            warn!(client_name = %client_name, "login attempt with revoked token");
            return Err(AuthError::TokenRevoked);
        }

        if client_status != "active" {
            warn!(client_name = %client_name, "login attempt for disabled client");
            return Err(AuthError::ClientDisabled);
        }

        Ok(AuthResult { client_name })
    }
    ...
    async fn rotate_token(&self, name: &str) -> Result<String> {
        ...
        let client_id: i64 =
            sqlx::query_scalar("SELECT id FROM clients WHERE name = ?")
                .bind(name)
                .fetch_one(&mut *tx)
                .await?;

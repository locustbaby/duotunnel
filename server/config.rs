...
pub use tunnel_store::server_config::{
    ...
    IngressListenerDef as IngressListener, IngressModeDef as IngressMode,
    IngressRouting, ServerConfigFile, ServerDef, ServerEgressUpstream,
    TcpListenerDef as TcpListenerConfig, TunnelManagement, UpstreamDef, VhostRuleDef as VhostRule,
};
...
impl ConfigSource for DbSource {
    async fn load(&self) -> Result<(TunnelManagement, ServerEgressUpstream)> {
        ...
        let listeners = data.ingress_listeners.into_iter().map(|l| IngressListener {
            port: l.port,
            mode: match l.mode {
                tunnel_store::IngressListenerMode::Http { vhost } => {
                    IngressMode::Http(HttpListenerConfig {
                        vhost: vhost.into_iter().map(|r| VhostRule {
                ...
                tunnel_store::IngressListenerMode::Tcp { group_id, proxy_name } => {
                    IngressMode::Tcp(TcpListenerConfig { client_group: group_id, proxy_name })
                }
            },
        }).collect();

        let groups = data.client_groups.into_iter().map(|g| {
            let upstreams = g.upstreams.into_iter().map(|u| (
                u.name,
                UpstreamDef {
                    servers: u.servers.into_iter().map(|s| ServerDef {
                        address: s.address, resolve: s.resolve,
                    }).collect(),
        ...
        let egress_upstreams = data.egress_upstreams.into_iter().map(|u| (
            u.name,
            UpstreamDef {
                servers: u.servers.into_iter().map(|s| ServerDef {
                    address: s.address, resolve: s.resolve,
        ...
        let egress = ServerEgressUpstream {
            ...
            rules: EgressRules {
                vhost: data.egress_vhost_rules.into_iter().map(|r| EgressHttpRule {
                    match_host: r.match_host,
        ...
        match self.primary.load().await {
            Ok((tm, egress)) => {
                if tm.is_empty() {
                    ...
                    let merged_egress = if egress.upstreams.is_empty() && egress.rules.vhost.is_empty() {
                        fallback_egress
                    } else {
                        egress
                    };
...
pub fn validate_server_config(cfg: &ServerConfigFile) -> Result<()> {
    ...
    for listener in &cfg.tunnel_management.server_ingress_routing.listeners {
        ...
        if !seen_ports.insert(listener.port) {
            errors.push(format!("tunnel_management: duplicate listener port {}", listener.port));
        }
        match &listener.mode {
            IngressMode::Http(h) => {
                ...
                for rule in &h.vhost {
                    if rule.match_host.is_empty() {
                        errors.push(format!("port {}: vhost rule has empty match_host", listener.port));
                    }
                    if !seen_hosts.insert(rule.match_host.as_str()) {
                        errors.push(format!(
                            "port {}: duplicate match_host \"{}\"", listener.port, rule.match_host
                        ));
                    }
...
pub fn build_client_config_for_group(
    ...
    let upstreams = group.upstreams.iter().map(|(name, def)| tunnel_lib::UpstreamConfig {
        name: name.clone(),

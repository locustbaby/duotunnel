...
impl MyTunnelServer {
    fn new_with_config(config: &crate::config::ServerConfig) -> Self {
...
impl Default for MyTunnelServer {
    fn default() -> Self {
        Self::new_with_config(&ServerConfig::load("../config/server.toml").unwrap())
    }
}
...
async fn handle_forward_proxy_from_client(
    ...
    if let Some(rule) = rules_engine.match_forward_rule(host, path, None) {
        if let Some(ref upstream_name) = rule.action_upstream {
            return forward_to_backend_server(&req, upstream_name).await;
        }
    }
    
    // 默认处理：直接代理
    if req.url.starts_with("http://") || req.url.starts_with("https://") {
        forward_to_backend_server(&req, &req.url).await
    } else {
        HttpResponse {
            status_code: 404,
            headers: HashMap::new(),
...
async fn forward_to_backend_server(req: &HttpRequest, target_url: &str) -> HttpResponse {
    use hyper::Client;
...
async fn main() -> anyhow::Result<()> {
    ...
    let tunnel_server = MyTunnelServer::new_with_config(&config);
        ...
        let make_svc = make_service_fn(move |_| {
            ...
            async move {
                Ok::<_, hyper::Error>(service_fn(move |req| {
                    ...
                    async move {
                        let host = req.headers().get("host").and_then(|h| h.to_str().ok()).unwrap_or("");
                        // gRPC service 名可从 header 或 path 提取，这里简化为 None
                        let service = None;
                        // 1. reverse_proxy 优先
    ...
    let rules_engine_tunnel = rules_engine.clone();
    let client_registry_tunnel = tunnel_server.client_registry.clone();
    let tunnel_server = tunnel_server; // 已有结构体

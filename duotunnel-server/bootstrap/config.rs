pub use duotunnel_lib::config::file::{
...
pub fn build_client_config_for_group(
    tm: &TunnelManagement,
    egress_rules: &[duotunnel_lib::EgressVhostRuleDef],
    group_id: &str,
    ...
    let upstreams = group
        .upstreams
        .iter()
        .map(|(name, def)| duotunnel_lib::UpstreamConfig {
            name: name.clone(),
            servers: def
                .servers
                .iter()
                .map(|s| duotunnel_lib::UpstreamServer {
        ...
        .collect();
    Some(duotunnel_lib::ClientConfig {

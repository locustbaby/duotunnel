...
use bytes::Bytes;
use duotunnel_lib::proxy::core::{Context, Protocol, UpstreamResolver};
use duotunnel_lib::proxy::http_connector::SharedHttpConnector;
use duotunnel_lib::proxy::peers::{BasicPeerSpec, HttpPeerSpec, PeerSpec, TlsPeerSpec};
use duotunnel_lib::proxy::tcp::UpstreamScheme;
use duotunnel_lib::proxy::upstream::UpstreamHealthRegistry;
use duotunnel_lib::{HttpClientParams, ProxyError, UpstreamGroup, VhostRouter};
...
impl ServerEgressMap {
    ...
    async fn resolve_healthy_target(
        ...
        (
            duotunnel_lib::proxy::upstream::BackendRef,
            std::net::SocketAddr,
        ),
        ProxyError,
    > {
        ...
        for _ in 0..group.servers.len() {
            ...
            let parsed = duotunnel_lib::transport::addr::parse_upstream(backend_ref.address());
    ...
    async fn connect_peer(
        ...
        match peer {
            PeerSpec::Tcp(spec) => {
                ...
                let (tcp_stream, final_tls) = if let Some(upstream_name) = &spec.upstream_name {
                    ...
                    for _ in 0..group.servers.len().max(1) {
                        let tcp_peer = current_spec
                            .clone()
                            .into_tcp_peer(duotunnel_lib::TcpParams::default())
                ...
                let tcp_params = duotunnel_lib::TcpParams::default();
                tcp_params
                    .apply(&tcp_stream)
                    .map_err(|e| ProxyError::upstream_connect(e.to_string()))?;

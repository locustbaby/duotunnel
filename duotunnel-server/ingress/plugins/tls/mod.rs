...
use duotunnel_lib::{EmptyBodyRetryTemplate, OpenStreamRequest, ProxyError, RouteTarget};
...
impl IngressProtocolHandler for TlsHandler {
    ...
    async fn handle(
        ...
        let server_config = duotunnel_lib::infra::pki::get_or_create_server_config(&host)
            .await
        ...
        let service = service_fn(move |req: Request<hyper::body::Incoming>| {
            ...
            async move {
                ...
                if !health.admits_new_work() {
                    ...
                    duotunnel_lib::plugin::observe_proxy_error(
                        metrics.as_ref(),
        ...
        if negotiated_h2 {
            ...
            let connection = duotunnel_lib::proxy::h2::hardened_h1_server_builder()
                .serve_connection(io, service);
            tokio::pin!(connection);
            tokio::select! {
                result = &mut connection => {

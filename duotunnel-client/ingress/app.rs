...
use bytes::Bytes;
use duotunnel_lib::plugin::{LoadBalancer, PickCtx, Resolver, Target};
use duotunnel_lib::proxy::core::{Context as ProxyContext, Protocol, UpstreamResolver};
use duotunnel_lib::proxy::http_connector::SharedHttpConnector;
use duotunnel_lib::proxy::peers::{
...
impl<L: LoadBalancer, R: Resolver> LocalProxyMap<L, R> {
        ...
        for upstream in &config.upstreams {
            ...
            let targets: Vec<Target> = raw
                ...
                .map(|addr| {
                    ...
                    let parsed = duotunnel_lib::transport::addr::parse_upstream(addr);
                    Target {
                        host: parsed.host,
                        port: parsed.port,
                        scheme,
    ...
    pub async fn resolve_addr(&self, connect_addr_str: &str) -> Result<SocketAddr> {
        ...
        let parsed = duotunnel_lib::transport::addr::parse_upstream(connect_addr_str);
    ...
    async fn connect_peer(
        ...
        match peer {
            ...
            PeerSpec::MitmH2(spec) => {
                let server_config = duotunnel_lib::get_or_create_server_config(&spec.tls_host)
                    .await
                    .map_err(|e| ProxyError::upstream_connect(e.to_string()))?;
                let acceptor = tokio_rustls::TlsAcceptor::from(server_config);
                let stream = duotunnel_lib::QuinnStream { send, recv };
                let stream = if let Some(init) = initial_data {
                    duotunnel_lib::PrefixedReadWrite::new(stream, init)
                } else {
                    duotunnel_lib::PrefixedReadWrite::new(stream, bytes::Bytes::new())
                };

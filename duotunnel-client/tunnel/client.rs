...
pub(crate) async fn run_client(
    ...
    let tcp_params = duotunnel_lib::TcpParams::from(&config.tcp);
    ...
    for _ in 0..UDP_REPLY_WORKERS {
        ...
        udp_workers.spawn(async move {
            let _tracked = duotunnel_lib::track_resource(duotunnel_lib::TrackedResource::UdpTask);
            loop {
                tokio::select! {
        ...
        tokio::select! {
            ...
            datagram_result = conn.read_datagram() => {
                match datagram_result {
                    Ok(payload) => {
                        match duotunnel_lib::decode_udp_datagram_envelope(payload.as_ref()) {
                            Ok(envelope) => {
...
fn udp_reply_worker(session: &duotunnel_lib::UdpSessionKey, workers: usize) -> usize {
...
async fn establish_session(
    ...
    let (mut send, mut recv) = duotunnel_lib::timeout(login_timeout, conn.open_bi())
        .await
    ...
    let msg_type = duotunnel_lib::timeout(login_timeout, recv_message_type(&mut recv))
        .await
    ...
    let resp: LoginResp = duotunnel_lib::timeout(login_timeout, recv_message(&mut recv))
        .await
...
async fn connect_to_server(
    ...
    for addr in addrs {
        ...
        match duotunnel_lib::timeout(connect_timeout, connecting).await {
...
async fn resolve_server_addresses(
    ...
    let resolved = duotunnel_lib::timeout(resolve_timeout, lookup)
        .await
        .map_err(|_| {

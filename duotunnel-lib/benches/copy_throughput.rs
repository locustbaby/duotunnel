...
use duotunnel_lib::engine::copy::{copy_buffered_then_finish, copy_buffered_then_shutdown};
use std::sync::Arc;
...
fn bench_copy_throughput(c: &mut Criterion) {
    ...
    let (client_conn, server_conn, _ce, _se) = rt.block_on(async {
        let (certs, key) = duotunnel_lib::infra::pki::generate_self_signed_cert().unwrap();
